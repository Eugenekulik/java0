...
public class CubeSpecificationByMinVolumeTest {
    ...
    @Test(description = "test cube specification by min volume", dataProvider = "input_data")
    public void specifiedTest(CubeRepository cubeRepository, Cube cube){
        List<Cube> expected = new ArrayList<Cube>();
        expected.add(cube);
        List<Cube> actual = cubeRepository.quary(new CubeSpecificationByMinVolume(4000));
