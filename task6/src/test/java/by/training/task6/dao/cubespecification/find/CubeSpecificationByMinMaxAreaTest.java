...
public class CubeSpecificationByMinMaxAreaTest {
    ...
    @Test(description = "test cube specification by min max area", dataProvider = "input_data")
    public void specifiedTest(CubeRepository cubeRepository, Cube cube){
        List<Cube> expected = new ArrayList<Cube>();
        expected.add(cube);
        List<Cube> actual = cubeRepository.quary(new CubeSpecificationByMinMaxArea(5,7));
